...
Enter the member key under **Settings > Club > Enter club key** to unlock self check-in without changing the Free or Pro news plan. Member access can see the current event and scan a card but cannot create or close events, inspect rosters or profiles, view historical meetings, delete records, or use developer and Pro news features. Developer access retains the complete operator workflow. The first supported card scan opens a registration form for name, phone number or Instagram handle, grade, and consent; later scans record attendance for the active event. Age is no longer collected or displayed, including for existing profiles. When Google Sheets sync is configured, the consent text discloses that the member's name, contact information, and attendance state are copied to the private club Sheet. Phone and Instagram have separate columns before the dated meetings.

Deploy the updated backend before distributing this iPhone build: registrations no longer send age, so the server must accept profiles without it. Previously encrypted profiles remain readable, and their retired age field is omitted from API responses.
...
The backend requires separate `CLUB_PAIRING_TOKEN` and `CLUB_DATA_KEY` values with at least 32 random characters each. It persists only a keyed fingerprint of the NFC identifier and encrypts the member profile at rest. Treat the data key as durable backup material: changing or losing it prevents existing records from being matched or decrypted. Rotate the pairing token if it is shared outside the club; rotation blocks future activation with the old key but does not revoke phones that already activated. The privacy manifest and public privacy policy declare the collected profile and card-derived data; update App Store Connect privacy answers before submitting a build containing this feature for public review.
